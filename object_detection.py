    ...
    for row in range(grid_h):
        for col in range(grid_w):
            for b in range(nb_box):
                x = _sigmoid(netout[row, col, b, 0])
                y = _sigmoid(netout[row, col, b, 1])
                w = netout[row, col, b, 2]
                h = netout[row, col, b, 3]

                objectness = _sigmoid(netout[row, col, b, 4])
                classes = _sigmoid(netout[row, col, b, 5:])
    ...
    for box in boxes:
        ...
        if label >= 0:
            ...
            if tf:
                ...
                cv2.waitKey(5)
                os.makedirs("Detected Images", exist_ok=True)
                out_path = os.path.join("Detected Images", "violation_" + str(dcnt) + ".jpg")
