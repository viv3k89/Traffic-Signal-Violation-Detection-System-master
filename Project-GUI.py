...
class Window(Frame):
    def __init__(self, master=None):
        ...
        if not os.path.exists(self.filename):
            blank = Image.new("RGB", (640, 480), (200, 200, 200))
            blank.save(self.filename)
    ...
    def open_file(self):
        self.filename = filedialog.askopenfilename(filetypes=[("Video Files", "*.mp4 *.avi *.mov")])
        ...
        preview_path = os.path.join('Images', 'preview.jpg')
    ...
    def imgClick(self, event):
        ...
        if self.counter == 2:
            ...
            print(self.line)
            print(self.rect)
            img = cv2.imread(os.path.join('Images', 'preview.jpg'))
            cv2.line(img, self.line[0], self.line[1], (0, 255, 0), 3)
            copy_path = os.path.join('Images', 'copy.jpg')
    ...
    def main_process(self):
        ...
        os.makedirs(os.path.join('Resources', 'output'), exist_ok=True)
        writer = imageio.get_writer(os.path.join('Resources', 'output', 'output.mp4'), fps=fps)
